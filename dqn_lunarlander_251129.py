...
def run_experiment(algorithm_type="DQN", render=False, load_model=False):
    ...
    if load_model and os.path.exists(model_path):
        ...
        try:
            ...
            # 이어서 학습할 경우, epsilon도 낮은 값부터 시작하도록 설정하여 활용 유도
            epsilon = 0.1 
    ...
    # 💡 에피소드 횟수를 10000에서 1000으로 수정
    for n_epi in range(1000): 
        s, _ = env.reset()
        done = False

        while not done:
            # s는 numpy 배열, torch.from_numpy로 텐서 변환 후 sample_action에 전달
    ...
    return q, q_target # 학습된 Q-네트워크 객체 반환
...
def evaluate_model(q_net, env_name='LunarLander-v3', num_episodes=100, render=False):
    ...
    for n_epi in range(num_episodes):
        ...
        with torch.no_grad(): # 평가 시에는 그래디언트 계산 불필요
            while not done:
                # sample_action은 epsilon=0.0일 때 argmax를 반환
        ...
        # 📌 성공률 기준: 최종 스코어가 200점 이상이면 성공으로 간주
...
def main():
    ...
    if choice == "1":
        q_net_to_evaluate, _ = run_experiment("DQN", render, load_model)
    elif choice == "2":
        q_net_to_evaluate, _ = run_experiment("Double_DQN", render, load_model)
    elif choice == "3":
        q_net_to_evaluate, _ = run_experiment("Dueling_DQN", render, load_model)
    elif choice == "4":
        for alg in algorithms:
            # 전체 실행 시, 각 알고리즘별로 학습 및 평가를 연속적으로 진행
            q_net, _ = run_experiment(alg, render, load_model)
            evaluate_model(q_net, render=False) # 평가 시에는 렌더링을 끄는 것이 일반적
        return
    else:
